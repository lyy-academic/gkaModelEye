function [outputRay, initialRay, targetIntersectError ,rayPath] = findGlintRay_lyy( worldOrigin, eyePose, worldTarget, rotationCenters, opticalSystemFixRL, opticalSystemRot, opticalSystemFixLR )
...
[outputRay,rayPath] = threeSystemTrace(initialRay', eyePose, rotationCenters, opticalSystemFixRL, opticalSystemRot, opticalSystemFixLR, Rstruc);
...
% outputRayStack=rotateEyeRay(outputRayStack,eyePose,rotationCenters,'inverse'); % this doesn't work as the function is only for 2*3 matrices
% for i=1:(size(outputRayStack,1)/2)
%     outputRayStack(2*i-1:2*i,:)=rotateEyeRay(outputRayStack(2*i-1:2*i,:),eyePose,rotationCenters,'inverse');
% end
...
rayPath=rayPath'; % here the raypath only comes from the rotating optical system, i.e. the cornea and lens
