function [pupilEllipse, glintCoord, imagePoints, worldPoints, headPoints, eyePoints, pointLabels, targetIntersectError, pupilFitError,rayPath_1st,rayPath_4th] = projectModelEye_lyy_parallel(eyePose, sceneGeometry, varargin)
...
p.addParameter('glintRayFunc',@findGlintRay_lyy,@(x)(isa(x,'function_handle') || isempty(x)));
p.addParameter('parallelRays',[],@iscell);
...
%% Add glint(s)
% [eyePoints, pointLabels,glintInitialRay_1st,rayPath_1st,outputRay_1st] = ...
%     addGlint_lyy_1st(eyePoints,pointLabels,sceneGeometry,p,eyePose);

%% lyy： add 4th Purkinje images
% [eyePoints, pointLabels, glintInitialRay_4th, rayPath_4th, outputRay_4th] = ...
%     addGlint_lyy_4th(eyePoints,pointLabels,sceneGeometry,p,eyePose);


%% lyy: add glints from parallel source
[eyePoints, pointLabels, rayPath_1st,rayPath_4th] = ...
    addGlint_lyy_parallel(eyePoints,pointLabels,sceneGeometry,p,eyePose,p.Results.parallelRays);
