function figHandle = plotModelEyeSchematic_lyy(eye, varargin)
...
p.addParameter('rayPath_1st',[],@(x)(isempty(x) || ismatrix(x)));
p.addParameter('rayPath_4th',[],@(x)(isempty(x) || ismatrix(x)));
...
if ~isempty(p.Results.rayPath_1st)
    if iscell(p.Results.rayPath_1st)
        for ii = 1:length(p.Results.rayPath_1st)
            to_plot=p.Results.rayPath_1st{ii}';
            plot(to_plot(PdimA,:),to_plot(PdimB,:),['x-' 'r']);
        end
    else
        to_plot=p.Results.rayPath_1st';
        plot(to_plot(PdimA,:),to_plot(PdimB,:),['x-' 'r']);
    end
    % plot(p.Results.rayPath_1st(PdimA,:),p.Results.rayPath_1st(PdimB,:),['x-' 'r']);
end
if ~isempty(p.Results.rayPath_4th)
    if iscell(p.Results.rayPath_4th)
        for ii = 1:length(p.Results.rayPath_4th)
            to_plot=p.Results.rayPath_4th{ii}';
            plot(to_plot(PdimA,:),to_plot(PdimB,:),['x-' 'b']);
        end
    else
        to_plot=p.Results.rayPath_4th';
        plot(to_plot(PdimA,:),to_plot(PdimB,:),['x-' 'b']);
    end
    % plot(p.Results.rayPath_4th(PdimA,:),p.Results.rayPath_4th(PdimB,:),['x-' 'b']);
end
