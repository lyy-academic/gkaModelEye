function [figHandle, plotHandles] = plotOpticalSystem_lyy(opticalSystem,options)
...
if ~isempty(options.rayPath_1st)
    if iscell(options.rayPath_1st)
        for ii=1:length(options.rayPath_1st)
            to_plot=options.rayPath_1st{ii}';
            plotHandles(end+1) = plot3(to_plot(1,:),to_plot(2,:),to_plot(3,:),'x-','Color','r');
        end
    elseif ismatrix(options.rayPath_1st)
        to_plot=options.rayPath_1st';
        plotHandles(end+1) = plot3(to_plot(1,:),to_plot(2,:),to_plot(3,:),'x-','Color','r');
    end
end
if ~isempty(options.rayPath_4th)
    if iscell(options.rayPath_4th)
        for ii=1:length(options.rayPath_4th)
            to_plot=options.rayPath_4th{ii}';
            plotHandles(end+1) = plot3(to_plot(1,:),to_plot(2,:),to_plot(3,:),'x-','Color','b');
        end
    elseif ismatrix(options.rayPath_4th)
        to_plot=options.rayPath_4th';
        plotHandles(end+1) = plot3(to_plot(1,:),to_plot(2,:),to_plot(3,:),'x-','Color','b');
    end
end
