function [pupilEllipse, glintCoord, imagePoints, worldPoints, headPoints, eyePoints, pointLabels, targetIntersectError, pupilFitError] = projectModelEye_translation(eyePose, sceneGeometry, varargin)
...
p.addParameter('translation_method',@bidirectional_linear,@(x)(isa(x,'function_handle') || isempty(x)));
p.addParameter('translation_params',struct(),@isstruct);
...
%% lyy: Apply eye translation
[headPoints, pointLabels] = applyEyeTranslation(headPoints, pointLabels, eyePose, p.Results.translation_method, p.Results.translation_params);
